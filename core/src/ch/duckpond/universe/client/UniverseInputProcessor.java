...
class UniverseInputProcessor extends InputAdapter {
    ...
    public boolean touchDown(final int screenX, final int screenY, final int pointer, final int button) {
        final Vector3 unprojectedPoint = universe.getCamera().unproject(new Vector3(screenX,
                                                                                    screenY,
                                                                                    0));
    ...
    public boolean touchDragged(final int screenX, final int screenY, final int pointer) {
        ...
        if (!universe.isMassSpawning()) {
            ...
            // set spawn velocity
            final Vector3 panPoint = universe.getCamera().unproject(new Vector3(screenX,
                                                                                screenY,
                                                                                0));
