...
public class UniverseGestureProcessor extends GestureDetector.GestureAdapter {
    ...
    public boolean tap(float x, float y, int count, int button) {
        ...
        gameScreen.setMassSpawnPointScreen(new Vector3(massSpawnPoint3.x, massSpawnPoint3.y, 0));
        return true;
    }
    ...
    public boolean pan(float x, float y, float deltaX, float deltaY) {
        if (!gameScreen.isMassSpawning()) {
            ...
            gameScreen.setMassSpawnVelocityScreen(new Vector3(gameScreen.getMassSpawnPointScreen().x - panPoint.x,
                                                              gameScreen.getMassSpawnPointScreen().y - panPoint.y,
                                                              0));
        }
        return true;
    }
